...
DIRECTORY="/w/hallb-scshelf2102/clas12/richcap/Exclusive_RG-A_Momentum_Corrections/Data_Files/Monte_Carlo_SIDIS/Pass2/" # Adjust the directory path as needed
FILE_PATTERN="Simulated_Single_Pion_Channel_epipX_Inbending_With_Dp_${VARIABLE_PART}_File"
INPUT_FILES="${FILE_PATTERN}_*root"
...
if [ "$ACTION" = "check" ]; then
    ...
    if [ $? -eq 0 ]; then
	    ...
	    # Use the find command to delete files matching INPUT_FILES pattern but excluding OUTPUT_FILE
            find . -wholename "${DIRECTORY}${INPUT_FILES}" ! -wholename "${DIRECTORY}${OUTPUT_FILE}" -exec rm -v {} \;
        else
